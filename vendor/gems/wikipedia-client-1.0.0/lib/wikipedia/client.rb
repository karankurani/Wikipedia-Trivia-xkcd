module Wikipedia
  class Client
    ...
    def random_article
      reply = request( {:action => "query",
                                 :prop => "info",
                                 :rvprop => "content",
                                 :list => "random",
                                } )
      json_string = JSON.parse(reply)      
      while (json_string["query"]["random"].first["title"].include? ":")
              reply = request( {:action => "query", :prop => "info", :rvprop => "content", :list => "random"} )
              json_string = JSON.parse(reply)              
      end      
      find(json_string["query"]["random"].first["title"])
    end
